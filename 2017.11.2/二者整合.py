...
while True:
    ...
    #------------------------账户属性的添加-----------------------------------------
        if '流动性' not in all_information['账户'][account_add]:
            account_current=input('请输入%s的流动性属性 [1]current assets [2]current liability [3]所有者权益:'%account_add)
            ...
            all_information['账户'][account_add].setdefault('流动性',[])
            all_information['账户'][account_add]['流动性'].append(account_current)
            all_information['账户'][account_add]['流动性'].append('0')
    ...
    while flag_all_account_edit:#一级目录 还得有最初的目录
        ...
        while True:#type_recond记录这里的信息.
            ...
            while True:#借贷双向记录
                ...
                if '属性' not in all_information['账户'][account_name]:
                    account_shuxing = input( "请输入%s的属性例如[1]assets [2]liability [3]revenues [4]expenses [5]sells [6]COGS [7]stockholders' equity:" % account_name)
                    if str(account_shuxing) == '1':
                        account_shuxing = '资产'
                    if str(account_shuxing) == '2':
                        account_shuxing = '负债'
                    ...
                    if str(account_shuxing) == '7':
                        account_shuxing = "所有者权益"
                    all_information['账户'][account_name].setdefault('属性', account_shuxing)
                    # ------------------------账户属性的添加-----------------------------------------
                if '流动性' not in all_information['账户'][account_name]:
                    account_current = input('请输入%s的流动性属性 [1]current assets [2]current liability [3]所有者权益:' % account_name)
                    if str(account_current) == '1':
                        account_current = '流动资产'
                    if str(account_current) == '2':
                        account_current = '流动负债'
                    if str(account_current) == '3':
                        account_current = '所有者权益排名'
                    all_information['账户'][account_name].setdefault('流动性', [])
                    all_information['账户'][account_name]['流动性'].append(account_current)
                    all_information['账户'][account_name]['流动性'].append('0')
                all_information['账户'][account_name].setdefault('debt', {})
                all_information['账户'][account_name].setdefault('credit', {})
                accounting_print(time_of_start)
